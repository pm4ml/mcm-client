...
class Server extends ws.Server {
    ...
    reconfigure({ logger = this._logger, port = 0, appConfig = this._appConfig }) {
        ...
        return () => {
            const reconfigureClientLogger = ({ logger: clientLogger }) => clientLogger.configure(logger);
    ...
    async notifyClientsOfCurrentConfig() {
        ...
        const logError = (socket, message) => (err) => this._logger
        ...
        const sendToAllClients = (msg) => Promise.all(
            [...this.clients.values()].map((socket) => (new Promise((resolve) => socket.send(msg, resolve))).catch(logError(socket, msg))),
        );
